...
public partial class Search : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            string checksearch = "SELECT COUNT(*) FROM Products WHERE Product_Name LIKE @search";
            ...
            if (temp > 0)
            {
                ...
                strCommandText += " FROM Products WHERE Product_Name LIKE @ProdName";
                SqlCommand cmd = new SqlCommand(strCommandText, myConnect);

                //declare cmd parameters for title and author to be dispayed
                cmd.Parameters.Add("@ProdName", SqlDbType.NVarChar, 50);
                cmd.Parameters["@ProdName"].Value = "%" + txtSearch.Text + "%";
