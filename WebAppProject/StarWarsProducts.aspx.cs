...
public partial class StarWarsProducts : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        DataTable dataStarWarsproducts = GetStarWarsProducts();


        //DataSource is used to pull the data from the database and populate them
        //step 8: pull data using DataSource
        Repeater1.DataSource = dataStarWarsproducts;
    ...
    private DataTable GetStarWarsProducts()
    {
        ...
        using (SqlConnection conn = new SqlConnection(SunnyCS))
        {
            ...
            SqlDataAdapter cmd = new SqlDataAdapter("SELECT * FROM Products WHERE Type = 'StarWars'", conn);

            //step 5: create a new DataSet
            DataTable dataStarWarsproducts = new DataTable();

            //step 6: pass the retrieved data into the newly created Dataset
            cmd.Fill(dataStarWarsproducts);


            //step 7: return
            return dataStarWarsproducts;
        }
    }
